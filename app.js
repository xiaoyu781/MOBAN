(() => {
  ...
  const DB_VERSION = 1;
  const STORE = 'foods';
  const FOOD_CACHE_KEY = 'np:foods-cache-v132';
  ...
  function fridgeView() {
    const categories=['全部','蔬菜','水果','肉蛋','豆制品','水产','主食','乳制品','其他'];
    ...
    if(state.fridgeFilter!=='全部') items=items.filter(({food})=>{
      ...
      if(state.fridgeFilter==='主食') return ['谷物/主食','坚果/种子'].includes(food.category);
      if(state.fridgeFilter==='乳制品') return food.category==='奶制品';
      if(state.fridgeFilter==='蔬菜') return food.category==='蔬菜';
      return !['蔬菜','水果','肉类','禽类','水产','蛋类','豆类/豆制品','谷物/主食','坚果/种子','奶制品'].includes(food.category);
    });
  ...
  function profileView() {
    ...
    return `<section class="screen page">
      <h1>我的</h1><p class="muted" style="margin-top:6px">身体数据、营养目标与本地数据管理</p>
      ...
      <div class="card data-card"><div class="data-status"><div><strong>Food Database（食材数据库）</strong><span>内置中文常用食材库 · ${state.foodMeta.count.toLocaleString()} 条</span></div><span class="badge">已就绪</span></div>
        <p class="muted" style="font-size:12px;line-height:1.55;margin-top:10px">支持中文名称、常用别名、英文名称搜索，并区分生、熟、干重与即食状态。营养值用于日常记录参考，品牌、品种和烹饪方式不同会产生差异。</p>
        <button class="secondary" type="button" data-action="browse-foods" style="width:100%;margin-top:12px">浏览 / 搜索食材库</button>
      </div>
      <div class="card data-card"><strong>本地数据备份</strong><p class="muted" style="font-size:12px;line-height:1.5;margin-top:6px">身体资料、冰箱库存和饮食记录只保存在当前手机浏览器。建议定期导出备份。</p><div class="form-actions"><button class="secondary" type="button" data-action="backup">导出备份</button><label class="secondary" style="display:flex;align-items:center;justify-content:center;cursor:pointer">恢复备份<input id="restoreInput" type="file" accept="application/json" hidden></label></div></div>
  ...
  const foodSearchCategories=['全部','蔬菜','水果','肉类','禽类','水产','蛋类','豆类/豆制品','谷物/主食','奶制品','坚果/种子','油脂','调味品','饮品'];
  ...
  function init() {
    // Render immediately from local data so a slow mobile connection cannot block the app shell.
    state.foodMeta = {mode: state.foods.length > 100 ? 'cached-zh-common' : 'starter-demo', count: state.foods.length};
    saveState();
    render();
    registerServiceWorker();
    // Full food data is a background enhancement. Re-render only when the refresh finishes.
    loadFoods().then(() => {
